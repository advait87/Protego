...
def backup_windows_state():
    """Exports current system state for rollback."""
    ...
    try:
        subprocess.run(f'reg export r"HKLM\Software\Microsoft\Windows\CurrentVersion\Policies\System" "{reg_path}" /y', 
                       shell=True, check=True, creationflags=subprocess.CREATE_NO_WINDOW)
